...
@connect(({ picture, loading }) => ({
  picture,
  loading: loading.effects['picture/detail'],
}))
export default class PictureDetail extends Component {
  ...
  render() {
    const { picture: { detail } } = this.props;
    return (
      <div>
        <Card title={`编号: ${detail.id}`} style={{ marginBottom: 24 }} bordered={false}>
          ...
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="投票数">{ detail.vote }</Description>
          </DescriptionList>  
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="作品名称">{ detail.name }</Description>
          </DescriptionList>       
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="作品描述">{ detail.description }</Description>
          </DescriptionList>       
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="拍摄日期">{ detail.date }</Description>
          </DescriptionList>       
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="原创">{ detail.original ? "是": "否" }</Description>
          </DescriptionList>       
          <DescriptionList style={{ marginBottom: 24 }}>
            <Description term="拍摄地址">{ detail.province + detail.city + detail.district + detail.address }</Description>
          </DescriptionList>       
          <DescriptionList style={{ marginBottom: 24 }}>
